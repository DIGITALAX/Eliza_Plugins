...
//     const targetUrl = `http://localhost:5000/e51f224d-70d3-0f8c-90d5-e456b6ab9822/message`;
...
//     const agentProcess = spawn(
//         "pnpm",
//         ["start-agent", "--port", , "--dbName", dbName],
//         {
//             cwd: projectRoot,
//             detached: true,
//             stdio: "ignore",
...
import { mainCharacter } from "./pixel.character";
import {
    AgentRuntime,
    CacheManager,
    CacheStore,
...
function initializeDatabase(dataDir: string, dbName: string) {
    const filePath = path.resolve(dataDir, dbName);
...
export async function createAgent(
    ...
    return new AgentRuntime({
        ...
        plugins: [nodePlugin, getSecret(character, "OPENAI_API_KEY")].filter(
            Boolean
        ),
        providers: [],
        actions: [],
...
async function startAgent(
    ...
    try {
        ...
        const dataDir = path.join("db.sqlite", "../data");

        // console.log(dataDir, dbName);
        if (!fs.existsSync(dataDir)) {
            fs.mkdirSync(dataDir, { recursive: true });
        }

        db = initializeDatabase(dataDir, "db.sqlite") as IDatabaseAdapter &
            IDatabaseCacheAdapter;
...
const startAgents = async () => {
    ...
    // if (!args?.port || !args?.dbName) {
    //     console.error("⚠️  Error: No se especificó puerto o base de datos");
    //     process.exit(1);
    // }
    ...
    directClient.start(3001);

    // if (args?.port !== parseInt(settings.SERVER_PORT || "3000")) {
    //     elizaLogger.log(`Server started on alternate port ${args?.port}`);
    // }
